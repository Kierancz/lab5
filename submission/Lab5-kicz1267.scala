object Lab5 extends jsy.util.JsyApplication {
  ...
  //From lab4: Finds the first element in L which when f applied to it returns some(x)
  // Just like mapFirst from Lab 4 but uses a callback f that returns a DoWith in the Some case.
  def mapFirstWith[W,A](f: A => Option[DoWith[W,A]])(l: List[A]): DoWith[W,List[A]] = l match {
    //if it reaches the end of the list, then it returns an unmodified version of the list passed in 
    case Nil => doreturn(l)
    //if the list actually has stuff in it, we look at the head
    case h :: t => f(h) match {
      //if the fist element doesn't match, recurse and map the list to a modified version with the head
      //attached to a modified tail
      case None => mapFirstWith(f)(t) map { x => List(h) ::: x }
      //if it does match, map the new modified alpha to the tail
  ...
  def castOk(t1: Typ, t2: Typ): Boolean = (t1, t2) match {
    ...
    case (TObj(fields1), TObj(fields2)) => 
      ...
      val check1 = fields2 forall {
        //Iterate through the elements in field 2. Compare them to their field1 couterparts
        //If the elements in each element-pair have the same type -> true
        //If a field1 counterpart is not found, or the elements in the pair do not match -> false
        case (field_i, t_i) => fields1.get(field_i) match {
      ...
      val check2 = fields1 forall {
        //Same as castOkUP, but instead of iterating through the elements in fields2, 
        //we iterate throught field1 and compare those against found fields2 counterparts.
        case (field_i, t_i) => fields2.get(field_i) match {
          case Some(t_2) => if (t_i == t_2) true else false
          case None => true
        }
      }
      //Return flase if either one is false
      check1 || check2
  ...
  def typeInfer(env: Map[String,(Mutability,Typ)], e: Expr): Typ = {
    ...
    e match {
      ...
      case Function(p, paramse, tann, e1) => {
        ...
        //We can use env2 and store env1 in it then iterate through env2
        ...
        val env2 = paramse match {
          ...
          //one extension, one variable (x) being passed
          //(mut, )
        }
      ...
      //Q: What does Mut mean on the info sheet?
      ...
      case Call(e1, args) => typ(e1) match {
        ...
        case TFunction(Left(params), tret) if (params.length == args.length) => {
          (params, args).zipped.foreach {
            (x1, x2) => 
              //check if the types of the args match the typ of the params
              if (x1._2 != typ(x2)) err(typ(x2), x2) else typ(x2)
          }
          tret
        }
        //TypeCallNameVar & TypeCallRef, Right = different mode
        case tgot @ TFunction(Right((mode, _, tparam)), tret) if (args.length == 1) =>
          val targ0 = typ(args(0))
          mode match {
            case PName | PVar =>
            //check to make sure that the type of parameters match what is given
            if (targ0 != tparam) 
              err(targ0, args(0))
            else
              tret
  ...
  val emp: Map[String, (Mutability, Typ)] = Map()
  def typeInfer(e: Expr): Typ = {
    typeInfer(emp, e)
  }
  ...
  def step(e: Expr): DoWith[Mem, Expr] = {
    ...
    e match {
      ...
      //SearchDecl
      case Decl(mut, x, e1, e2) if !isValue(e1) =>
        for (e1p <- step(e1)) yield Decl(mut, x, e1p, e2)
      //SearchObj
      case Obj(fields) => fields find { case (_, ei) => !isValue(ei) } match {

        case Some((fi, ei)) =>
          step(ei) map { e => Obj(fields + (fi -> e)) }
        case None => println("matched Obj None"); throw StuckError(e)
      }
      //SearchGetField
      case GetField(e1, f) => {
      ...
      case Call(e1, args) =>
        println("matched searchCall...args: " + args); e1 match {

          case f @ Function(_, Left(_), _, _) => // SearchCall2
            println("matched searchCall2")
            mapFirstWith {
              (arg: Expr) =>
                if (isValue(arg)) None
                else Some(step(arg))
          ...
          case f @ Function(_, Right((PVar, _, _)), _, _) if !isValue(args(0)) =>
            println("matched searchCallVar: " + args(0))
            step(args(0)) map { newargs => Call(f, List(newargs)) }
          case f @ Function(_, Right((PRef, _, _)), _, _) if !isValue(args(0)) && !isLValue(args(0)) =>
            println("matched searchCallRef")
            step(args(0)) map { newargs => Call(f, List(newargs)) }

          case a @ _ if !isValue(a) => //SearchCall1
            println("matched searchCall1")
            val eip_with = step(e1);
